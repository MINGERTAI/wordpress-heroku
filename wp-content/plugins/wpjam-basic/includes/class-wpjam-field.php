...
class WPJAM_Field{
	...
	public  function parse_json_schema_enum($enum, $type='integer'){
		if($type == 'integer'){
			return array_map('intval', $enum);
		}elseif($type == 'number'){
			return array_map('floatval', $enum);
		}else{
	...
	public  function prepare($args=[]){
		...
		if(isset($schema['enum'])){
			$schema['enum']	= $this->parse_json_schema_enum($schema['enum'], $schema['type']);
		}

		if(isset($schema['items']['enum'])){
			$schema['items']['enum']	= $this->parse_json_schema_enum($schema['items']['enum'], $schema['items']['type']);
		}

		if(isset($schema['properties'])){
			foreach($schema['properties'] as &$schema_property){
				if(isset($schema_property['enum'])){
					$schema_property['enum']	= $this->parse_json_schema_enum($schema_property['enum'], $schema_property['type']);
				}
			}
		}

		if(isset($schema['items']['properties'])){
			foreach($schema['items']['properties'] as &$schema_property){
				if(isset($schema_property['enum'])){
					$schema_property['enum']	= $this->parse_json_schema_enum($schema_property['enum'], $schema_property['type']);
				}
			}
