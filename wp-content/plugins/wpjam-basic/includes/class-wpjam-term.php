...
class WPJAM_Term_Option{
	...
	public function get_fields($term_id=null){
		if(is_callable($this->args)){
			...
			$this->fields	= [$this->name => wpjam_array_except($this->args, 'taxonomy')];
		}
	...
	public function prepare_value($term_id){
		...
		return $this->get_fields_object($term_id)->prepare([
			'value_callback'	=> ['WPJAM_Term', 'value_callback'], 
			'id'				=> $term_id
		]);
	}

	public function list_action($term_id, $data){
		if($fields = $this->get_fields($term_id)){
			$defaults	= (new WPJAM_Fields($fields))->get_defaults();
		}else{
			$defaults	= null;
		}

		return wpjam_update_metadata('term', $term_id, $data, $defaults);
	}
